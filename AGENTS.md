...
MCP endpoint + token). All arguments are optional: identity resolves from
`SITE_TITLE`/`SITE_URL`/`SITE_DESCRIPTION` env vars (or generic defaults),
and the target dir defaults to `./agentic-blog-site`. Foreground process
manager — Ctrl+C stops both servers. This is the one-command path for an
agent that receives the repo URL and must stand up a connected blog.